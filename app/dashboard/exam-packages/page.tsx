...
function ExamPackageCard({
  ...
  return (
    <GlassCard className={cn("relative flex flex-col h-full transition-all duration-300 hover:scale-[1.02]")}>
      {isPurchased && (
        <div className="absolute top-4 left-4 px-2.5 py-1 rounded-full bg-success/20 border border-success/30 text-success text-xs font-medium">
          Unlocked
        </div>
      )}
...
export default function ExamPackagesPage() {
  ...
  const [countsById, setCountsById] = useState<Record<string, { flashcards: number; questions: number; simulations: number }>>({})
  const [trackStatus, setTrackStatus] = useState<"active" | "coming_soon">("active")
  ...
  useEffect(() => {
    ...
    const loadTrackStatus = async () => {
      try {
        const res = await fetch("/api/track-status", { cache: "no-store" })
        if (!res.ok) return
        const data = await res.json()
        if (active && data?.status) setTrackStatus(data.status)
      } catch {
        // no-op
      }
    }
  ...
  useEffect(() => {
    async function fetchCounts() {
      try {
        ...
        const data = await res.json()
        setCountsById(data.counts || {})
      } catch (err) {
        console.error("Failed to fetch package counts", err)
      }
    }
  ...
  const handlePurchase = async (pkg: typeof examPackages[0]) => {
    if (trackStatus === "coming_soon") return
    try {
        ...
        {examPackages.map((pkg) => (
          <ExamPackageCard
            ...
            pkg={{
              ...pkg,
              stats: {
                flashcards: countsById[pkg.id]?.flashcards ?? pkg.stats.flashcards,
                questions: countsById[pkg.id]?.questions ?? pkg.stats.questions,
                cases: countsById[pkg.id]?.simulations ?? pkg.stats.cases,
              },
              features: [
                `${countsById[pkg.id]?.flashcards ?? pkg.stats.flashcards}+ Flashcards`,
                `${countsById[pkg.id]?.questions ?? pkg.stats.questions} Practice Questions`,
                `${countsById[pkg.id]?.simulations ?? pkg.stats.cases} Case Simulations`,
                pkg.features[3] || "Full Practice Exams",
              ],
            }}
            isPurchased={purchasedPackages.includes(pkg.id)}
            onPurchase={handlePurchase}
            canPurchase={trackStatus !== "coming_soon"}
          />
        ))}
      </div>
