...
const examPackages = [
  {
    ...
    stats: { flashcards: 150, questions: 200, cases: 15, practiceExams: 3 },
  },
  {
    id: "paramedic",
    ...
    stats: { flashcards: 200, questions: 280, cases: 20, practiceExams: 4 },
  },
  {
    id: "nclex",
    ...
    stats: { flashcards: 250, questions: 350, cases: 18, practiceExams: 5 },
  },
  {
    id: "mcat",
    ...
    stats: { flashcards: 300, questions: 400, cases: 12, practiceExams: 6 },
  },
  {
    id: "usmle",
    ...
    stats: { flashcards: 350, questions: 500, cases: 25, practiceExams: 8 },
  },
]
...
export default function ExamPackagesPage() {
  ...
  useEffect(() => {
    ...
    const loadPurchases = async () => {
      try {
        ...
        if (active) setPurchasedPackages(data.purchasedExamIds || [])
      } catch (err) {
  ...
  const packagesWithCounts = examPackages.map((pkg) => {
    const counts = countsMap[pkg.id]
    if (!counts) return pkg
    return {
      ...pkg,
      stats: {
        ...pkg.stats,
        ...
        cases: counts.simulations,
        practiceExams: counts.practiceExams,
      },
      features: [
        `${counts.flashcards}+ Flashcards`,
        `${counts.questions} Practice Questions`,
        `${counts.simulations} Case Simulations`,
        `${counts.practiceExams} Full Practice Exams`,
      ],
    }
  })
