...
function ExamProgramCard({
  ...
  return (
    <GlassCard className="relative overflow-hidden transition-all duration-300 hover:border-primary/30 card-hover">
        {isPurchased && (
          <div className="absolute top-4 left-4 px-2.5 py-1 rounded-full bg-success/20 border border-success/30 text-success text-xs font-medium z-10">
            Unlocked
          </div>
        )}
        {/* Price badge */}
            ...
            <div key={program.id} className="animate-fade-in-up" style={{ animationDelay: `${index * 100}ms` }}>
              <ExamProgramCard 
                program={{
                  ...program,
                  phases: [
                    { ...program.phases[0], items: countsById[program.id]?.flashcards ?? program.phases[0].items },
                    { ...program.phases[1], items: countsById[program.id]?.questions ?? program.phases[1].items },
                    { ...program.phases[2], items: countsById[program.id]?.simulations ?? program.phases[2].items },
                    program.phases[3],
                  ],
                }} 
