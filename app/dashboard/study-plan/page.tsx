...
async function getStudyPlanData(userId: string) {
  ...
  // Get or create study plan tasks for today
  ...
  if (!existingTasks || existingTasks.length === 0) {
    const flashcardTarget = Math.min((dueFlashcards?.length || 0) + (newFlashcards?.length || 0), 30)
    const questionTarget = Math.min(recommendedQuestions?.length || 0, 15)
    const caseTarget = Math.min(availableCases?.length || 0, 2)
...
export default async function StudyPlanPage() {
  ...
  const taskMap = new Map(data.tasks.map((t: any) => [t.task_type, t]))

  const dailyGoal = {
    flashcards: { target: taskMap.get("flashcards")?.target_count || 20, current: flashcardsReviewed },
    questions: { target: taskMap.get("questions")?.target_count || 10, current: questionsAnswered },
    cases: { target: taskMap.get("cases")?.target_count || 1, current: casesCompleted },
  }
