...
export default function LeaderboardPage() {
  ...
  useEffect(() => {
    async function fetchLeaderboard() {
      ...
      const { data: profiles } = await supabase
        .from("profiles")
        .select("id, username, avatar_url, study_streak, level, program_type")
        .in("id", userIds)
      ...
      if (trackFilter !== "all") {
        filteredUserIds = filteredUserIds.filter((id) => profileById.get(id)?.program_type === trackFilter)
      }

      // Apply cohort filter for current user's classes
      if (cohortFilter === "my_cohort" && user) {
        ...
        if (classIds.length > 0) {
          ...
          const cohortIds = new Set((cohortStudents || []).map(c => c.student_id))
          filteredUserIds = filteredUserIds.filter((id) => cohortIds.has(id))
        }
      }

      // Build leaderboard
      const entries: LeaderboardEntry[] = filteredUserIds
        .map(userId => {
    ...
    fetchLeaderboard()
  }, [timeFilter, trackFilter, cohortFilter])
  ...
  const trackOptions = [
    ...
    { value: "emt_basic", label: "EMT" },
    { value: "paramedic", label: "Paramedic" },
    { value: "nursing", label: "Nursing" },
    { value: "pre_med", label: "Pre-Med" },
    { value: "medical_student", label: "Med Student" },
  ]
    ...
    <div className="space-y-8 pt-12 lg:pt-0">
      ...
      <div className="flex flex-wrap gap-2">
        {trackOptions.map((option) => (
          <Button
            key={option.value}
            variant={trackFilter === option.value ? "default" : "outline"}
            size="sm"
            onClick={() => setTrackFilter(option.value)}
            className={trackFilter === option.value ? "bg-primary text-primary-foreground" : ""}
          >
            {option.label}
          </Button>
        ))}
        <Button
          variant={cohortFilter === "my_cohort" ? "default" : "outline"}
          size="sm"
          onClick={() => setCohortFilter(cohortFilter === "my_cohort" ? "all" : "my_cohort")}
          className={cohortFilter === "my_cohort" ? "bg-primary text-primary-foreground" : ""}
        >
          My Cohort
        </Button>
      </div>
